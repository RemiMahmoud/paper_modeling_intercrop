...
For each model, set data is making 4 folds within each experiment to perform the cross validation. Then, the model is fitted within specific chunks (as this may be long to run).
...
```{r, eval = FALSE}
models_yield_legume_wt_fababean_1_30 <- data_kv_wt_fababean %>% 
  slice(1:30) %>% 
  mutate(mod.MERF =  map(data_train, fun_call_MERF_modified, verbose_param = TRUE, max_iterations =  3000, toll_param = 0.002, y = "yield_legume")) 





models_yield_legume_wt_fababean_31_39 <- data_kv_wt_fababean %>% 
  slice(31:39) %>% 
  mutate(mod.MERF =  map(data_train, fun_call_MERF_modified, verbose_param = TRUE, max_iterations =  3000, toll_param = 0.002, y = "yield_legume")) 

models_yield_legume_wt_fababean_40 <- data_kv_wt_fababean %>% 
  slice(40) %>% 
  mutate(mod.MERF =  map(data_train, fun_call_MERF_modified, verbose_param = TRUE, max_iterations =  3000, toll_param = 0.002, y = "yield_legume")) 



models_yield_legume_wt_fababean <- bind_rows(models_yield_legume_wt_fababean_1_30, models_yield_legume_wt_fababean_31_39)
