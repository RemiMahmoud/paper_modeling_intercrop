...
tibble_error_models_plot <- tibble_error_models %>% 
  mutate(species_mix = str_replace_all(species_mix, c( "wt_pea"  = "Blé dur / pois", "wt_fababean" = "Blé dur / Féverole", "all_species" = "Toutes espèces")) ) %>% 
  mutate(y = str_replace_all(y, c("_"= " ", "cereal" = "blé", "legume" = "lég", "yield" = "Rdt" ))) %>% 
  mutate(subset = str_replace_all(subset, c("all" ="Toutes les données", "test" = "Ensemble d'évaluation"))) %>% 
  rename("Espèces" = "species_mix", "Données" = "subset")
...
(
  plot_error_models <- tibble_error_models_plot %>% 
    ...
    ggplot(aes(x =y , y = value, group = interaction(Espèces, Données), color = Données)) +
...
ggsave(filename = "figures/MERF/error_models_cv_all.png", plot =plot_error_models, dpi = "retina", height = 12, width = 17, unit = "cm")
...
table_variance_imputation = tibble_var_imputation_models_plot %>% mutate(variance_imputation = paste0(round(100*variance_imputation, digits = 1), "%")) %>% pivot_wider(names_from = y, values_from = variance_imputation)
...
kable(table_variance_imputation, booktabs = TRUE,  escape = TRUE, format = "latex", longtable = T, label = "table_variance_imputation", caption = "Part de la variance due aux imputations") %>% 
