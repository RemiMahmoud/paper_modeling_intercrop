...
plot_importance_all_models <- function(list_models, vec_predictors = all_predictors, remove_unused_predictors = FALSE, add_species_mix = FALSE){
  ...
  # leg_id <- str_extract(model, "pea|fababean")
  ...
  # data_all_predictors$model_formatted <- str_replace_all(data_all_predictors$model, c("_"=" ", "PLER cereal" = "PLER blé\n", "PLER legume" = "PLER légumineuse\n", "yield"= "Rendement\n", "cereal"= "blé", "legume" = "lég"))
  ...
  plot_output <- data_all_predictors %>% 
    ...
    geom_label(aes(label = label_cor), size= 3) +
...
# remove_weird_suffix_reorder_within <- function(factor) {
#   # factor <- str_replace(factor, pattern = "__(.*)", ""))
#   # factor <- 
# }
...
ggsave(filename = "figures/MERF/plot_importance_all_models.png", plot =plot_importance_all_models, height = 24, width = 20, unit = "cm")
...
# 
list_models_environment <- str_remove_all(list_models, "models/N_seed/MERF/wt_pea/|models/N_seed/MERF/all_species/|models/N_seed/MERF/wt_fababean/|.rds")
