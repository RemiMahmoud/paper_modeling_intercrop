...
library(purrr)
library(broom)
library(nls.multstart) #nls with multiple start
...
data_spline_raw_height_final %>% select(experiment_id, crop_id, asymp, result, rmse, xslope, slope, index_slope, y_at_slope, lambda) %>% readr::write_rds("R/models/data_height_fit.rds")
data_spline_raw_B_shoot_final %>% select(experiment_id, crop_id, asymp, result, rmse, xslope, slope, index_slope, y_at_slope, lambda) %>% readr::write_rds("R/models/data_biomass_fit.rds")
data_spline_raw_cover_final %>% select(experiment_id, crop_id, asymp, result, rmse, xslope, slope, index_slope, y_at_slope, lambda) %>% readr::write_rds("R/models/data_cover_fit.rds")
```
