...
```{r tab FR}
data_N_residual <- tibble(experiment_id = c("Auzeville_cochard_2010", "Auzeville_PP_2011", "Auzeville_SGs_2007","Auzeville_TE_2006", "Auzeville_TO_2013", "Auzeville_ZN_2012"), N_resid = c(35,53,100,45,41,36))
...
table_environment <- environment %>%
  ...
  mutate(experiment_id = str_replace_all(experiment_id, c("Auzeville" = "Auz", "_cochard" =""))) %>% 
  arrange(species_mix, year) %>% 
  distinct("Mélange d'espèces" = species_mix,"Expérimentation"= experiment_id,  "Somme des précipitations (mm)" = SRR, "Température moyenne (°C)" = TM, "Reliquat N (kg/ha)" = N_resid, "Fertilisation N (kg/ha)" = N_fertilisation, "Date de semis" = sowing_date, "Date de récolte" = harvest_date, "Nb. d'observations"= n) 
...
sink("doc/table_environment.tex")
kable(table_environment, booktabs = TRUE,  escape = TRUE, format = "latex", longtable = T, label = "table_environement_chap_modeling", caption = "Conditions environnementales et caractéristiques des expérimentations") %>% 
  # add_header_above(c(" ", "colLabel1"=2, "colLabel2"=2)) %>% 
