...
bind_rows(data_explore_wt_fababean, data_explore_wt_pea)%>% 
  filter(variable == "cover") %>% 
  ggplot(aes(x = time_thermal, y = value)) +
  geom_point(alpha = .4) + 
  geom_line(alpha=  .4, aes(group = crop_id , color = species))+
  # geom_vline(data= data_flowering_wt_fababean, aes(xintercept = time_flowering_cereal), alpha = .5 , linetype = "dashed", color = "green") + 
  geom_vline(data= bind_rows(data_flowering_wt_fababean,data_flowering_wt_pea), aes(xintercept = time_thermal_flowering_legume), alpha = .5 , linetype = "dashed", color = "red", size =1.5) +
