...
data_compute_NNI <- bind_rows(traits_mean, traits_crop) %>% 
  ...
  mutate(ac_sp = case_when(
    ...
    stringr::str_detect(species, "wheat") & season == "winter" ~ 5.4
  )) %>% 
...
# data_nitrogen_shoot_recomputed <- data_compute_NNI_IC_SC %>% filter(is.na(nitrogen_shoot_component)) %>% select(experiment_id:date, biomass_shoot_component,  nitrogen_abs_shoot_component, biomass_shoot_component ) %>% 
#   mutate(nitrogen_shoot_component = 0.1* nitrogen_abs_shoot_component/biomass_shoot_component)
#Table1
...
#          Nc_comp_spi = (ac_sp_i*biomass_shoot_component^bc_sp_i),
#          Nc_mix_spi = (ac_sp_i*biomass_shoot_total^bc_sp_i))  %>% 
#   mutate(NNI_bedou = (nitrogen_shoot_component)/Nc_bedou_spi,
#          NNI_comp = (nitrogen_shoot_component)/Nc_comp_spi,
