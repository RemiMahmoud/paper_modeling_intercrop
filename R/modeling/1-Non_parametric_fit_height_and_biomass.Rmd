...
```{r fit all cover curves, eval = FALSE}
...
curves_auz_zn_2012 <- data_spline_raw_height %>% filter(experiment_id == "Auzeville_ZN_2012") %>%
  filter(str_detect(crop_id, "IC")) %>% distinct(crop_id)
# function_plot_curves(curves_auz_zn_2012, data_pred_smooth_splines_height, data_coordinates_slope_height, data_height)

data_pred_smooth_splines_height %>%
  inner_join( curves_auz_zn_2012) %>%
  left_join(data_height %>% select(crop_id, management, experiment_id, crop_type, species,observed =value, observed_time_thermal = time_thermal)) %>%
  left_join(data_coordinates_slope_height) %>%
  mutate(crop_id = forcats::fct_reorder(crop_id, slope, .desc = TRUE)) %>%
  ggplot(aes(x = time_thermal)) +
  # geom_line(aes(y = value, group = interaction( crop_id), color = rmse)) +
  geom_line(aes(y = value, group = interaction( crop_id), color = species)) +
  geom_point(aes(x = observed_time_thermal, y = observed ))+
  geom_point(aes(x = xslope, y= y_at_slope),
             color = "red", shape = 4, size = 3)+
...
curves_auz_TO_2013 <- data_spline_raw_height %>% filter(experiment_id == "Auzeville_TO_2013") %>% 
  filter(str_detect(crop_id, "M58|M1_|M34")) %>% distinct(crop_id)
# function_plot_curves(curves_auz_TO_2013, data_pred_smooth_splines_height, data_coordinates_slope_height, data_height)
 
data_pred_smooth_splines_height %>%
  inner_join( curves_auz_TO_2013) %>% 
  left_join(data_height %>% select(crop_id, management, experiment_id, crop_type, species,observed =value, observed_time_thermal = time_thermal)) %>% 
  left_join(data_coordinates_slope_height) %>% 
  mutate(crop_id = forcats::fct_reorder(crop_id, slope, .desc = TRUE)) %>% 
  ggplot(aes(x = time_thermal)) + 
  geom_line(aes(y = value, group = interaction( crop_id), color = slope)) + 
  geom_point(aes(x = observed_time_thermal, y = observed ))+
  geom_point(aes(x = xslope, y= y_at_slope),
             color = "red", shape = 4, size = 3)+
  facet_wrap(crop_id~. , scales= "free") + 
...
curves_auz_SGs_2007 <- data_spline_raw_height %>% filter(experiment_id == "Auzeville_SGs_2007") %>% 
  filter(str_detect(crop_id, "M35")) %>% distinct(crop_id)
# function_plot_curves(curves_auz_TO_2013, data_pred_smooth_splines_height, data_coordinates_slope_height, data_height)
 
data_pred_smooth_splines_height %>%
  inner_join( curves_auz_SGs_2007) %>% 
  left_join(data_height %>% select(crop_id, management, experiment_id, crop_type, species,observed =value, observed_time_thermal = time_thermal)) %>% 
  left_join(data_coordinates_slope_height) %>% 
  mutate(crop_id = forcats::fct_reorder(crop_id, slope, .desc = TRUE)) %>% 
  ggplot(aes(x = time_thermal)) + 
...
curves_not_well_estimated_B_shoot <-  tibble(crop_id = c("M32_2010_cochard_IC_wheat_turgidum_L1823","M21_2010_cochard_IC_wheat_turgidum_L1823","M25_2010_cochard_IC_wheat_turgidum_L1823", "M1_2007_SGs_SC_wheat_turgidum_acalou", "M28_2010_cochard_IC_fababean_castel", "M52_2013_TO_IC_pea_kazar","M25_2007_SGs_SC_wheat_turgidum_orjaune", "M9_2007_SGs_SC_wheat_turgidum_nefer"))
...
# data_spline_raw_B_shoot_final_old<- readr::read_rds("data/data_biomass_fit.rds")
...
data_management_control_SC <- readr::read_rds("data/management_control_IC_SC.rds") %>% 
  mutate(management_control = ifelse(plant_family == "legume",
                                     management_SC_N0_control,
                                     management_SC_control))
data_management_control_SC %>% filter( experiment_id == "Auzeville_SGs_2007",
          management %in% c("M39", "M47"))

data.intercrop::traits_mean %>% 
  filter( experiment_id == "Auzeville_SGs_2007",
          management %in% c("M39","M46", "M47", "M9", "M25", "M33"),
          variable == "biomass_shoot") %>% distinct(crop_id) %>% pull %>% dput

function_plot_curves(tibble(crop_id  = c("M39_2007_SGs_IC_pea_lucy","M46_2007_SGs_IC_pea_lucy", "M39_2007_SGs_IC_wheat_turgidum_nefer","M47_2007_SGs_IC_pea_lucy",
                                         "M47_2007_SGs_IC_wheat_turgidum_orjaune", "M25_2007_SGs_SC_wheat_turgidum_orjaune", "M33_2007_SGs_SC_pea_lucy", "M9_2007_SGs_SC_wheat_turgidum_nefer"
)), data_pred_smooth_splines_B_shoot, data_coordinates_slope_B_shoot, data_B_shoot) +scale_x_continuous(limits =c(0,2500))

data_pred_smooth_splines_B_shoot %>% filter(crop_id %in% c("M39_2007_SGs_IC_pea_lucy", "M39_2007_SGs_IC_wheat_turgidum_nefer","M47_2007_SGs_IC_pea_lucy",
                                         "M47_2007_SGs_IC_wheat_turgidum_orjaune", "M25_2007_SGs_SC_wheat_turgidum_orjaune", "M33_2007_SGs_SC_pea_lucy", "M9_2007_SGs_SC_wheat_turgidum_nefer")) %>% distinct(crop_id, lambda)

data_spline_raw_B_shoot_final_old%>% filter(crop_id %in% c("M39_2007_SGs_IC_pea_lucy", "M39_2007_SGs_IC_wheat_turgidum_nefer","M47_2007_SGs_IC_pea_lucy",
                                         "M47_2007_SGs_IC_wheat_turgidum_orjaune", "M25_2007_SGs_SC_wheat_turgidum_orjaune", "M33_2007_SGs_SC_pea_lucy", "M9_2007_SGs_SC_wheat_turgidum_nefer")) %>% distinct(crop_id, lambda)
...
data_management_control_SC <- readr::read_rds("data/management_control_IC_SC.rds") %>% 
  mutate(management_control = ifelse(plant_family == "legume",
                                     management_SC_N0_control,
                                     management_SC_control))
data_pred_smooth_splines_height <- data_pred_smooth(data_spline_raw_height_final %>% filter(crop_id %in% c("M46_2007_SGs_IC_wheat_turgidum_neodur", "M20_2007_SGs_SC_wheat_turgidum_neodur")), data_height)

function_plot_curves(tibble(crop_id  = c("M46_2007_SGs_IC_wheat_turgidum_neodur" , "M20_2007_SGs_SC_wheat_turgidum_neodur" )), data_pred_smooth_splines_height, data_coordinates_slope_height, data_height) + scale_y_continuous(limits = c(0,1)) + scale_x_continuous(limits = c(0,1700))

data_spline_raw_height_final %>% filter(crop_id %in% c("M46_2007_SGs_IC_wheat_turgidum_neodur" , "M20_2007_SGs_SC_wheat_turgidum_neodur")) %>% distinct(crop_id, lambda)
...
data_pred_smooth_splines_height <- data_pred_smooth(data_spline_raw_height_final %>% filter(crop_id %in% c("M24_2012_ZN_IC_wheat_turgidum_L1823", "M24_2012_ZN_IC_pea_geronimo","M25_2012_ZN_IC_pea_isard", "M25_2012_ZN_IC_wheat_turgidum_L1823", "M26_2012_ZN_IC_pea_lucy", "M26_2012_ZN_IC_wheat_turgidum_L1823")), data_height)


curves_manuscript <- tibble(crop_id  = c("M24_2012_ZN_IC_wheat_turgidum_L1823", "M24_2012_ZN_IC_pea_geronimo","M25_2012_ZN_IC_pea_isard", "M25_2012_ZN_IC_wheat_turgidum_L1823", "M26_2012_ZN_IC_pea_lucy", "M26_2012_ZN_IC_wheat_turgidum_L1823"))


(plot_manuscript <- data_pred_smooth_splines_height %>% 
  inner_join( curves_manuscript) %>%
  left_join(data_height %>% select(crop_id, management, experiment_id, crop_type, species,observed =value, observed_time_thermal = time_thermal)) %>%
  left_join(data_coordinates_slope_height) %>%
  mutate(crop_id = forcats::fct_reorder(crop_id, slope, .desc = TRUE)) %>%
  mutate(species= str_replace_all(species, c("pea" = "pois", 'wheat turgidum' = "blé"))) %>% 
  ggplot(aes(x = time_thermal)) +
  # geom_line(aes(y = value, group = interaction( crop_id), color = rmse)) +
  geom_line(aes(y = value, group = interaction( crop_id), color = species), linewidth = 1, alpha = .4) +
  geom_point(aes(x = observed_time_thermal, y = observed, color = species ), size = 3)+
  geom_point(aes(x = xslope, y= y_at_slope, color =species),
             shape = 4, size = 3)+
  facet_wrap(management~. , scales= "free") +
  labs(y = "Hauteur (m)", x = "Temps thermique (°Cd)", color = "Espèces") + 
  theme(legend.position = "bottom", strip.text.x = element_text(size = 8), axis.text =  element_text(size = 7), axis.title =  element_text(size =8), legend.title =  element_text(size = 8)))

ggsave("figures/plot_height_pea_wheat_auz_2012.png", plot = plot_manuscript, height = 8, width = 15, unit = "cm", dpi = "retina")

file.copy("figures/plot_height_pea_wheat_auz_2012.png", to  ="C:/Users/rmahmoud/Dropbox/Applications/Overleaf/Manuscrit thèse/chap4_modeling_IC/figures/plot_height_pea_wheat_auz_2012.png", overwrite = TRUE)
# 
